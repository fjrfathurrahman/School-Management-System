...
import { ExcelActions, SheetComponent } from './data-table-actions';

/**
 * Props untuk DataTableToolbar
 */
interface DataTableToolbarProps<TData> extends React.ComponentProps<'div'> {
    table: Table<TData>;
    // Props untuk button tambah data
    sheet?: {
        title: string;
        description?: string;
        component: React.ComponentType<unknown>;
    };
    // Props untuk export/import Excel
    excelActions?: {
        onExport?: () => void;
...
/**
 * Enhanced DataTableToolbar dengan Add, Export, dan Import functionality
 */
export function DataTableToolbar<TData>({ table, children, className, sheet, excelActions, ...props }: DataTableToolbarProps<TData>) {
    ...
    return (
        <div role="toolbar" aria-orientation="horizontal" className={cn('flex w-full items-start justify-between gap-2 p-1', className)} {...props}>
            ...
            <div className="flex items-center gap-2">
                ...
                {sheet && <SheetComponent {...sheet} />}

                {/* Excel Actions */}
                {excelActions && (
                    <ExcelActions
                        onExport={excelActions.onExport}
                        onImportTemplate={excelActions.onImportTemplate}
                        onImportData={excelActions.onImportData}
                    />
                )}
            </div>
        </div>
